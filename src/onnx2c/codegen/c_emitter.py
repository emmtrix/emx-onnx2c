...
class CEmitter:
    ...
    def emit_model(self, model: LoweredModel, *, emit_testbench: bool = False) -> str:
        ...
        constants_section = self._emit_constants(model.constants, model.dtype)
    ...
    def _emit_model_wrapper(
        ...
        *,
        ...
        signature = ", ".join(
            f"const {c_type} {name}{self._array_suffix(shape)}"
    ...
    def _emit_testbench(self, model: LoweredModel, testbench_template) -> str:
        ...
        for name, shape, count in zip(
            model.input_names, model.input_shapes, input_counts
        ):
        ...
        return testbench_template.render(
            ...
            c_type=dtype.c_type,
            zero_literal=dtype.zero_literal,
            print_format=self._print_format(model.dtype),
            print_cast=self._print_cast(model.dtype),
        ).rstrip()

    def _emit_constants(self, constants: tuple[ConstTensor, ...], dtype: str) -> str:
        if not constants:
            return ""
        c_type = dtype_info(dtype).c_type
        lines: list[str] = []
        for const in constants:
            array_suffix = self._array_suffix(const.shape)
            values = [
                self._format_value(value, dtype) for value in const.data
            ]
