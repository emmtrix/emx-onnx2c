...
class CEmitter:
    ...
    def emit_model(self, model: LoweredModel, *, emit_testbench: bool = False) -> str:
        ...
        concat_runtime = ""
        if any(isinstance(op, ConcatOp) for op in resolved_ops):
            concat_runtime = self._load_runtime_concat()
        if concat_runtime:
            sections.extend((concat_runtime.rstrip(), ""))
    ...
    @staticmethod
    def _resolve_op(
        op: BinaryOp | UnaryOp | MatMulOp | AttentionOp | ConvOp | ConcatOp,
    ...
    @staticmethod
    def _load_runtime_concat() -> str:
        runtime_dir = Path(__file__).resolve().parents[1] / "runtime"
        header = (runtime_dir / "concat.h").read_text(encoding="utf-8")
        source = (runtime_dir / "concat.c").read_text(encoding="utf-8")
        source_lines = [
            line for line in source.splitlines() if line.strip() != '#include "concat.h"'
        ]
        return "\n".join([header.rstrip(), "", *source_lines]).strip()
